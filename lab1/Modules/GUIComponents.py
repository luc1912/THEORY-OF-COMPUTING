...
class LanguageChooser:
    options = {
        ...
        ".jar": lambda args: ["java", "-jar", *args],
        # [*args[0].split(maxsplits=1), *args[1:]], # [tkfiles.askopenfilename(title="script that runs your java")]
    };
...
class FileTypes:
    def init(self, frame, *args) -> None:
		...
		""") + "\n\n" + "\n".join(label + ": " + description for (var, label, description) in args),
                          "Help on File Types");
...
class Output:
    def init(self, frame):
        ...
        yscroll = tk.Scrollbar(frame, orient=tk.VERTICAL, command=output.yview, );
        yscroll.grid(row=2, column=3, sticky=tk.NS);
        output['yscrollcommand'] = yscroll.set;
        xscroll = tk.Scrollbar(frame, orient=tk.HORIZONTAL, command=output.xview, );
        xscroll.grid(row=3, column=2, sticky=tk.EW);
        output['xscrollcommand'] = xscroll.set;

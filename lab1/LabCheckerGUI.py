...
class LabCheckerGUI(Tester):
    ...
    def init(self):
        ...
        self.timeout.init(config_frame, "timeout", "seconds",
                          lambda: tk.DoubleVar(config_frame, config["defaults"]["timeout"]),
                          "in case there is infinite loop, terminate program when time exceedes limit");
    ...
    def defFileTypes(self):
        self.file_types = {
            "input": (tk.StringVar(self.window.config_frame, config["defaults"]["extensions"]["input"]), "input files",
                      "program input"),
            "expected": (
            tk.StringVar(self.window.config_frame, config["defaults"]["extensions"]["expected"]), "their output",
            "expected output, will not be modified"),
            "output": (
            tk.StringVar(self.window.config_frame, config["defaults"]["extensions"]["output"]), "your output",
            "output file, will be modified, will be created if doesn't exist"),
            "error": (tk.StringVar(self.window.config_frame, config["defaults"]["extensions"]["error"]), "error output",
                      "file for writing errors, will be modified, will be created if doesn't exist"),
        }
    ...
    @property
    def test_files(self):
        yield from self._test_files_generator();
